...
export const fnv1a = (u8Buffer: Uint8Array, size: FNV_SIZE): BigInt => {
  const prime = FNV_PRIMES[size];
  let hash = FNV_OFFSETS[size];

  for (let i = 0; i < u8Buffer.length; i++) {
    hash ^= BigInt(u8Buffer[i]);
...
export const fnv1a_on_string = (key: string, size: FNV_SIZE): BigInt => {
  ...
  const u8Buffer = new Uint8Array();

  let remaining = key;

  while (remaining.length > 0) {
    const result = encoder.encodeInto(remaining, u8Buffer);
